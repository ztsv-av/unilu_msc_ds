...
% IMPORTANT: make sure to have 'Im1.jpg' and 'signal_hw.bin' and the same repository with this file.
...
figure
plot(n,fr);
title("Inverse of signal f: obtain signal fr")
...
% The second function is defined approximately on the interval n2=[300:800] with of period of 5.
...
%   we can deduce that we have a sum of exactly these two functions on the interval [300:499],
...
%   the second frequency is present around the interval [300:800] and there is an overlap of these two frequences
%   around the interval [300:499].
...
n1 = [0:298]; % define first function on interval [0:298]
n2 = [300:499]; % define overlap of two functions on interval [300:499]
n3 = [500:800]; % define second function on interval [500:800]
f = [sin(pi/5*n1) sin(pi/5*n2)+2*sin(4*pi/5*n2) 2*sin(4*pi/5*n3)];

% Here we repeat steps from questions 1 to 2,
%   i.e plot the signal, compute its FT, plot FT
N = length(f)
n = [0:N-1];
...
%   (for original singal f(0) = approximately 0.588).
% For sin(x) = approximately 0.588, x=pi/5.
% Thus, we need to phase shift the first signal by pi/5.
% We get:

n1 = [0:298]; % define first function on interval [0:298]
n2 = [300:499]; % define overlap of two functions on interval [300:499]
n3 = [500:800]; % define second function on interval [500:799]
f = [sin(pi/5*n1 + pi/5) sin(pi/5*n2)+2*sin(4*pi/5*n2) 2*sin(4*pi/5*n3)]; % define deduced signal f

% Here we repeat steps from questions 1 to 3,
%   i.e plot the signal, compute its FT, plot FT,
%   inverse signal, compute FT of inversed signal, plot it
N = length(f)
n = [0:N-1];
...
figure
plot(n,fr);
title("Inverse of deduced signal")
...
set(gca, "xlim",[0 freq(end)])
title("Amplitude |Ffr| of deduced inversed signal on [0,pi]")
subplot(2,1,2), plot(freq, angle(Ffr)); grid
set(gca, "xlim",[0 freq(end)])
title("Phase(Ffr) of deduced inversed signal on [0,pi]")

% Our deduced signal (and its inverse) is exactly the same as of the original.
